...
static void encode_wav_to_fafr(const Args& a) {
  SF_INFO info{};
  SNDFILE* in = sf_open(a.in_path.c_str(), SFM_READ, &info);
  ...
  apply_fade_in_out(audio_interleaved, channels, sr, a.fade_ms);

  const uint32_t N = a.frame_size;
  const uint32_t H = a.hop_size;
  ...
  // Open output
  std::ofstream out(a.out_path, std::ios::binary);
  ...
  std::cout << "Encoded:\n"
            << "  Input:  " << a.in_path << "\n"
            << "  Output: " << a.out_path << "\n"
            << "  SR: " << sr << " Hz, Channels: " << channels << "\n"
            << "  Frames: " << num_frames << " (frame=" << N << ", hop=" << H << ")\n"
            << "  Fade: " << a.fade_ms << " ms\n";
}

static void decode_fafr_to_wav(const Args& a) {
  std::ifstream in(a.in_path, std::ios::binary);
  ...
  SNDFILE* out = sf_open(a.out_path.c_str(), SFM_WRITE, &outinfo);
